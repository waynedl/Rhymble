...
function Round(){
    ...
    R.submit_guess = function(g){
	var guess = g.toUpperCase();
	var slot = $('.slot[word='+guess+'] SPAN');
	if (slot.length > 0){
	    slot.removeClass('hidden_slot')
	}
    };
...
function Game(){
    ...
    G.n_rounds = 10;
    G.time = 5;  //seconds 
    G.current_guess = new Array();  // should be on the Round class
	...
	$('.key').bind('tap', function(e){
		...
		G.this_round.submit_guess(G.current_guess.join(''));
